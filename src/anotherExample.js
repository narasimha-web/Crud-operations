const data = [
  ...
  function addingProduct(addingData){
    const addproduct = data.find(item => item.user_id === addingData.user_id)
    if(!addproduct){
...
console.log(addingProduct(employdata))

console.log(data)

// update product

function updateProduct(employId,employdat){
const index = data.findIndex(item => item.user_id == employId);
if(index !== -1){
 data[index] = {...data[index],...employdat}
}else{
...
const data1={
  ...
  "email": "narasimha@gmail.com",
  "given_name": "hulk",
  "family_name": "Userdefination",
}
console.log(updateProduct("584a9d13e808bcf75f05f0",data1))

//delete data

function deleteData(dataId){
const index = data.findIndex(item => item.user_id == dataId)
if(index !== -1){
  data.splice(index,1)
}else{
...
console.log(deleteData("584a9d13e808bcf75f05f580"))
